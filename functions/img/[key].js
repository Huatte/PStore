...
export async function onRequestGet(context) {
  ...
  const upstream = `https://raw.githubusercontent.com/${repo}/${branch}/images/${key}`;
  ...
  try {
    ...
    if (width > 0 && width <= 3000) {
      try {
        ...
        // resizing unavailable -> fall through to original
      }
    }

    // Fallback: original image (no resize or resizing not available)
